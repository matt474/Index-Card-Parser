	...
	<xsl:template match="reference">
		<xsl:if test="author | title | journal | booktitle">
			<mods xmlns="http://www.loc.gov/mods/v3" xmlns:mods="http://www.loc.gov/mods/v3"
									...
									<xsl:choose>
										<xsl:when test="contains(volume,'(')">
											<detail type="volume">
												<number>
												<xsl:value-of
												select="substring-before(volume, '(')"/>
												</number>
											</detail>
											<detail type="issue">
												<number>
									...
									<extent unit="pages">
										<xsl:choose>
											<xsl:when test="contains(pages,'-')">
												<start>
												<xsl:value-of
												select="normalize-space(substring-before(pages, '-'))"
												/>
												</start>
												<end>
												<xsl:value-of
												select="normalize-space(substring-after(pages, '-'))"
												/>
												</end>
											</xsl:when>
											<xsl:otherwise>
				...
				<xsl:choose>
					<xsl:when test="/references/reference/header">
						<xsl:if test="not(starts-with(/references/reference/header,'-'))">
							<topic>
								<subject>
									<xsl:value-of select="/references/reference/header"/>
								</subject>
							</topic>] <xsl:if test="footer">
								<location>
